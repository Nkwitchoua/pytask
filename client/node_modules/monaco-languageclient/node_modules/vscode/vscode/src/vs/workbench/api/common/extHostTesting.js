...
let ExtHostTesting = class ExtHostTesting extends Disposable {
    constructor(rpc, commands, editors) {
        ...
        this.activeProfilesChangedEmitter = this._register(( new Emitter()));
...
class TestRunProfileImpl {
    ...
    get onDidChangeSelected() {
        checkProposedApiEnabled(this.#extension, 'testingActiveProfile');
        return Event.chain(this.#onDidChangeActiveProfiles, $ => ( $
            .map(ev => ev.get(this.controllerId)?.get(this.profileId)))
            .filter(isDefined));
    }
    constructor(proxy, profiles, extension, activeProfiles, onDidChangeActiveProfiles, controllerId, profileId, _label, kind, runHandler, _isDefault = false, _tag = undefined, _supportsContinuousRun = false) {
        this.controllerId = controllerId;
