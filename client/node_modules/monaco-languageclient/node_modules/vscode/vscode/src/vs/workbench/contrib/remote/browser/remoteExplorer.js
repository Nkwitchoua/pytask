...
import { TUNNEL_VIEW_ID, TUNNEL_VIEW_CONTAINER_ID, PORT_AUTO_SOURCE_SETTING, PORT_AUTO_SOURCE_SETTING_PROCESS, PORT_AUTO_SOURCE_SETTING_HYBRID, PORT_AUTO_SOURCE_SETTING_OUTPUT, PORT_AUTO_FORWARD_SETTING, IRemoteExplorerService } from '../../../services/remote/common/remoteExplorerService.js';
...
import { OpenPortInPreviewAction, OpenPortInBrowserAction, TunnelPanel, openPreviewEnabledContext, ForwardPortAction, TunnelPanelDescriptor, TunnelViewModel } from './tunnelView.js';
...
const CAN_PROC_FALLBACK = 'processPortForwardingFallback';
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
    constructor(contextKeyService, environmentService, remoteExplorerService, tunnelService, activityService, statusbarService) {
        ...
        this._register(( ( Registry.as(Extensions.ViewsRegistry)).registerViewWelcomeContent(TUNNEL_VIEW_ID, {
            content: this.environmentService.remoteAuthority ? ( nls.localizeWithPath(
    ...
    async getViewContainer() {
        return ( Registry.as(Extensions.ViewContainersRegistry)).registerViewContainer({
            ...
            title: { value: ( nls.localizeWithPath('vs/workbench/contrib/remote/browser/remoteExplorer', 'ports', "Ports")), original: 'Ports' },
            icon: portsViewIcon,
...
let AutomaticPortForwarding = class AutomaticPortForwarding extends Disposable {
    ...
    canFallbackToHybrid() {
        return this.storageService.getBoolean(CAN_PROC_FALLBACK, 1 , true);
    }
    listenForPorts() {
            ...
            this.portListener = this._register(this.remoteExplorerService.tunnelModel.onForwardPort(async () => {
                if (Array.from(( this.remoteExplorerService.tunnelModel.forwarded.values())).filter(tunnel => tunnel.source.source === TunnelSource.Auto).length > 20) {
                    ...
                    this.notificationService.notify({
                        ...
                        actions: {
                            primary: [
                                ( new Action('switchBack', ( nls.localizeWithPath(
                                    'vs/workbench/contrib/remote/browser/remoteExplorer',
                                    ...
                                    this.storageService.store(CAN_PROC_FALLBACK, false, 1 , 0 );
                                    this.portListener?.dispose();
                                    this.portListener = undefined;
                                })),
                                ( new Action('showPortSourceSetting', ( nls.localizeWithPath(
                                    'vs/workbench/contrib/remote/browser/remoteExplorer',
...
export { AutomaticPortForwarding, CAN_PROC_FALLBACK, ForwardedPortsView, PortRestore, VIEWLET_ID };
