...
let CodeActionOnSaveParticipant = class CodeActionOnSaveParticipant {
    ...
    async participate(model, env, progress, token) {
        ...
        if (env.reason !== 1  && Array.isArray(setting)) {
            return undefined;
        }
        ...
        const filteredSaveList = Array.isArray(setting) ? codeActionsOnSave : codeActionsOnSave.filter(x => setting[x.value] === 'always' || (((setting[x.value] === 'explicit' || setting[x.value] === true) && env.reason === 1) ));
